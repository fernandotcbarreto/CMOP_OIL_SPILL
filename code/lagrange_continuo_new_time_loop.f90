...
!!!!!
  ...
  OPEN(225 , FILE = "zf2_19.txt" , STATUS= 'UNKNOWN', FORM= 'FORMATTED ' )
...
!  OPEN(27 , FILE = "mask.txt" , STATUS= 'UNKNOWN', FORM= 'FORMATTED ' )
  ...
  DISSOLVE = 1    ! 0 TURN OFF DISSOLUTION
  EMULSI  = 1    !0 TURN OFF EMULSIFICATION
  EVAP_TURN = 1    !0 TURN OFF EVAPORATION
  ENTRAIN= 1
...
!  !print*, windsp
  ...
  numpal=1!number of particles
  mwf= 933.45 !g/mol
  dt=600! TS always in second

  dt_random = 1

  ts = 12 !number of hours you wanna run

  time_finish = 1000000  ! number of hours that lasts the continuous release

  ts= (ts*60*60)/dt 

  tsl= 240000000 !minutes interval for realesing particle
...
!  !print*, y_model(1,:)
 ...
 IF (ENTRAIN.EQ.1) THEN
   ...
   write(331,fmt3) dropdiamf2(:,1)
   write(332,fmt3) spmtf2(:,1)
...
lat_partf2(m1,i-1), out_int )
        vi2 = out_int(1)
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t1_1d , power, 1, lon_partf2(m1,i-1), &
lat_partf2(m1,i-1), out_int )
        ti1 = out_int(1)
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t2_1d , power, 1, lon_partf2(m1,i-1), &
lat_partf2(m1,i-1), out_int )
        ti2 = out_int(1)
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s1_1d , power, 1, lon_partf2(m1,i-1), &
lat_partf2(m1,i-1), out_int )
        si1 = out_int(1)
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s2_1d , power, 1, lon_partf2(m1,i-1), &
lat_partf2(m1,i-1), out_int )
        si2 = out_int(1)
...
lat_partf2(m1,i-1), out_int )
 ...
 lat_partf2(m1,i-1), out_int_2 )
        ...
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t1_1d , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int )
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t1_1d_2 , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int_2 )
        vel_interp(1) = out_int(1)
        vel_interp(2) = out_int_2(1)       
        call pwl_value_1d (size(inter_depth), inter_depth, vel_interp, 1, par_dep , ui_vec )
        ...
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t2_1d , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int )
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, t2_1d_2 , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int_2 )
        vel_interp(1) = out_int(1)
        vel_interp(2) = out_int_2(1)       
        call pwl_value_1d (size(inter_depth), inter_depth, vel_interp, 1, par_dep , ui_vec )
        ...
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s1_1d , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int )
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s1_1d_2 , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int_2 )
        vel_interp(1) = out_int(1)
        vel_interp(2) = out_int_2(1)       
        call pwl_value_1d (size(inter_depth), inter_depth, vel_interp, 1, par_dep , ui_vec )
        ...
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s2_1d , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int )
        call shepard_interp_2d ( size(lon_1d), lon_1d, lat_1d, s2_1d_2 , power, 1, lon_partf2(m1,i-1), &
 lat_partf2(m1,i-1), out_int_2 )
        vel_interp(1) = out_int(1)
        vel_interp(2) = out_int_2(1)       
        call pwl_value_1d (size(inter_depth), inter_depth, vel_interp, 1, par_dep , ui_vec )
...
lat_partf2(m1,i-1), out_int_2 )
      ...
      ti1 = t_model1 (lat_in_f2(1), lon_in_f2(2), indexz(1))
      ti2 = t_model2 (lat_in_f2(1), lon_in_f2(2), indexz(1))
      si1 = s_model1 (lat_in_f2(1), lon_in_f2(2), indexz(1))
      si2 = s_model2 (lat_in_f2(1), lon_in_f2(2), indexz(1))       
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! DISSOLUTION FASE 2
      temp_outf2 = tif2 + 273.15
      SAL_Af2 = sif2

!     print*, 'ts f2', m1,temp_outf2, SAL_Af2

       call  PROP_AMBIENTE(zf2(m1,i-1) , PROF_REF , temp_outf2 , SAL_Af2, RO_A , VIS_DIN_A , CP_A , TS_A)

       call VEL_ASCENSAO_BG ( dropdiamf2(M1,i-1) , DIAM_HYD , RO_A , spmtf2(m1,i-1), VIS_DIN_A  , TS_VC  , Wvp  )

print*, 'm1', m1, spmtf2(m1,i-1)

          CALL PROPRIEDADES_COMP_OIL ( TEMP_OUTf2 , PM_COMP_OIL(:)	 , &
		         TEB_COMP_OIL(:) , RO_COMP_OIL_15(:) , &
		         RO_COMP_OIL(:) , CP_COMP_OIL(:) )
         ...
         CALL components_part2( API , VAZAO_OIL_OUT , DT , TEMP_OUTf2 , &
	         	RO_OIL_OUT , PM_OIL_OUT , TS_OIL_OUT , VIS_DIN_OIL_OUT , TS_A, TS_VC)
...
!        !print*, 'klk', temp_outf2 , SAL_Af2
        ...
        call  PROP_AMBIENTE(zf2(m1,i-1) , PROF_REF , temp_outf2 , SAL_Af2, RO_A , VIS_DIN_A , CP_A , TS_A)   !!gotta update Z to zf2(m1,i-1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           call  PROP_AMBIENTE(Z , PROF_REF , temp_outf2 , SAL_Af2, RO_A , VIS_DIN_A , CP_A , TS_A)
...
((RO_A - RO_OIL_OUT )/RO_A ) ) )**(1./3.)  )
 ...
 !           diam(NUMTOT, i)=2*(((massa(NUMTOT, i)/spmt)/(pi*height(NUMTOT, i)))**0.5)  !meter                    
            ...
            height(NUMTOT, i) = vol(NUMTOT, i) / area(NUMTOT, i)
             ...
             massae(NUMTOT, i) = massa(NUMTOT, i)
...
!print*, dt_hf2, limen, limassem, massa(j,i)
   ...
   if ((qd.gt.0) .and. (dt_hf2 .ge.limen) .and. (massa(j,i).ge. limassem)) then
 ...
 !    write(23,fmt2) porc_evap(:,i)
    ...
    close(769)
     ...
     if (ENTRAIN.EQ.1) THEN 
       ...
       write(331,fmt3) dropdiamf2(:,i)
       write(332,fmt3) spmtf2(:,i)       
